 ...
 */
 ...
 function init_data() {
    ...
    this.possible_attacks[utils.hash_obj_tuple(this.track_user,this.user_w_cookies)] =  {source: 5, target: -1};
    this.possible_defenses[utils.hash_obj_tuple(this.track_user,this.use_non_tracing_services)] = {source:-15, target:10};

    this.possible_attacks[utils.hash_obj_tuple(this.steal_user_photos,this.user_non_rights_aware)] = {source:10,target: -1};
    this.possible_defenses[utils.hash_obj_tuple(this.steal_user_photos,this.user_read_fine_prints)] = {source:-15,target:10};

    this.possible_attacks[utils.hash_obj_tuple(this.exploit_user,this.user_not_doing_updates)]= {source:10,target:-2};
    this.possible_defenses[utils.hash_obj_tuple(this.exploit_user,this.user_updates_patches)]={source:-5,target:20};

    this.possible_attacks[utils.hash_obj_tuple(this.steal_user_password,this.user_using_weak_passwords)] = {source:20,target:-2};
    this.possible_defenses[utils.hash_obj_tuple(this.steal_user_password,this.user_use_two_factor_auth)] = {source:-5, target:30};

    this.possible_attacks[utils.hash_obj_tuple(this.steal_sensitive_data,this.user_not_crypting_emails)] = {source:20,target:-2};
